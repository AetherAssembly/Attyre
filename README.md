...
[![Version](https://img.shields.io/badge/version-v3.0.1-C9A96E)](CHANGELOG.md)
...
Run the NSIS installer (`Attyre_3.0.1_x64-setup.exe`) and follow the prompts, or use the `.msi` for silent/managed installs.
...
```bash
sudo apt install ./Attyre_3.0.1_amd64.deb
```
...
```bash
sudo dnf install ./Attyre-3.0.1-1.x86_64.rpm
```
...
```bash
sudo zypper install ./Attyre-3.0.1-1.x86_64.rpm
```
